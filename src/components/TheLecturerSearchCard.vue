<template>
	<v-card class="my-4 position-relative" :height="height" variant="elevated" rounded="xl">
		<template #prepend>
...
onMounted(() => {
	subjectsToShow.value =
		props.lecturer.subjects !== null ? props.lecturer.subjects.filter((item: string) => item !== null) : null;
	displaySubjects.value = subjectsToShow.value && subjectsToShow.value.length > 0;
